...
TRAINER_KNOWLEDGE_INSTRUCTION = (
    ...
    "The teaching, if any, is in what the EMPLOYEE explicitly STATES as true - often as a correction "
    "or addition such as 'actually...', 'for your information...', 'note that...', or 'remember "
    "that...', frequently right after the assistant said something was not in the documents.\n\n"
    "Capture ONLY facts the EMPLOYEE explicitly asserts, and write them as a clear, self-contained "
    "statement (a short paragraph) suitable as induction material.\n\n"
    "Strict rules:\n"
    "- A QUESTION is NOT knowledge. If the employee only ASKS about a topic (for example 'what is "
    "X?', 'how does Y work?', 'is there a Z policy?'), there is nothing to capture - EVEN IF you "
    "happen to know the answer. Do NOT define, answer, or expand the topic.\n"
    "- Do NOT use any outside or general knowledge, and do NOT infer or invent details the employee "
    "did not state. Capture only what the employee actually wrote.\n"
    "- Treat the EMPLOYEE's assertion as the authority. The assistant may have hedged - called "
    "something 'unofficial', said it 'cannot be confirmed from the documents', or suggested checking "
    "with management. That hedging is EXPECTED and does NOT mean there is no knowledge; ignore it. "
    "But do NOT capture facts that only the ASSISTANT stated.\n\n"
    "If the employee did not explicitly assert any fact or guidance (for example they only asked "
