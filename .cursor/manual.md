...
3. Frontend deps installed:
   ...
   npm install
   ```
4. `.env` filled in (OpenAI key, Cohere key, JWT secret, admin email/password, and for the Opus 4.8 answer model: `LLM_PROVIDER=anthropic` + `ANTHROPIC_API_KEY` + `ANTHROPIC_CHAT_MODEL=claude-opus-4-8`; the fast lane `FAST_LLM_PROVIDER=openai` + `FAST_CHAT_MODEL=gpt-4o-mini`). Already done. The answer is generated by Claude Opus 4.8; all mechanical steps run on `gpt-4o-mini`.
