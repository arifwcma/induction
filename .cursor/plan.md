...
- New table `PendingIngest` (auto-creates on startup). New endpoints: `POST /kb/summarise`, `POST /kb/trainer-file`; `POST /kb/document` repurposed to staging. Ingest is **auto-applied** via a FastAPI background task (the manual `GET /kb/pending` + `POST /kb/apply-pending` were removed — it just happens a few seconds after save/upload). Deploy = `update.sh` (no re-ingest), but the server compose must add the `induction_trainer_docs` volume first. Eval 11/11, smoke Cases 1–12 clean after the change.
- Admin visibility/download/delete (later add): `GET /admin/kb` merges applied trainer files (`PendingIngest`) with legacy `TrainerKBEntry` rows (newest-first, each with a `downloadable` flag); `GET /admin/kb/{id}/download` streams the stored file (cookie auth, original name for documents); `DELETE /admin/kb/{id}` fully purges a trainer file by its `source` filename — Qdrant points + BM25 records + `Clause` rows + the on-disk file + the row, then refreshes caches.
