...
- A left-panel **Apply pending (N)** button (`POST /kb/apply-pending`) ingests every pending file IN-PROCESS via the refactored `embed_and_index_file` (the `ingest_one` core: Qdrant + BM25 + clause table), marks each applied (idempotent — never re-ingests an applied file), then resets the BM25 + KB-outline caches so the knowledge is retrievable without a container restart (the container can't restart itself).
