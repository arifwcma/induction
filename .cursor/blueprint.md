...
The KB is four static files in `documents/` (env `DOCUMENTS_DIR=documents`). Ingestion picks up `*.pdf` and `*.docx` (skips `~$` temp files):
1. `Wimmera CMA Enterprise Agreement 2024-2028.pdf` (~945 KB) — the hard one: hierarchical clauses 1–47 + Appendices A–D; conditional provisions (Appendix C Emergency Work). This drives almost every reliability mechanism.
2. `General Office Procedures Practices.docx` (~535 KB) — office hours, locking up, visitors, dress, phones/voicemail, email, calendars, templates.
3. `Induction Policy.docx` (~343 KB) — first-week induction process.
4. `Uniform Policy.docx` (~347 KB).

A full ingest of these produces ~385 contextual chunks and ~368 clause rows. `.doc` (old binary Word) is NOT cleanly ingestible — convert to `.docx`. New/edited documents are dropped into `documents/` and a re-ingest is run.
...
flowchart TD
  ...
  subgraph backend [FastAPI backend]
    ...
    Pipe["condense -> split -> rewrite -> retrieve -> rerank -> expand -> applicability -> generate -> verify -> (agentic re-retrieve)"]
    Stores["session / prompt / kb / clause stores"]
  end
  Postgres[("PostgreSQL: users, sessions, messages, prompt, trainer_kb, clauses")]
...
7. Condense (fast lane): rewrite history + message into a standalone question. Carry the prior topic forward ONLY for bare situation/condition follow-ups ("what would be the case during emergency work" → "During emergency work, does a meal break count as worked hours?"); NEVER for topic switches ("lets talk about the short tours").
...
11. Generic scope/precedence system prompt (NO hardcoded keywords): obey any condition stated in a passage's context; a conditional clause governs only if its condition is met; apply precedence (NES-overrides per EA clause 4.3; specific-over-general). MAP authoritative for existence/coverage; SOURCE MATERIAL required for substantive facts; overview-first for broad topics; self-aware guided tour; out-of-scope politely deflected.
...
18. Eval harness (`app/eval_harness.py`, first-class): adversarial multi-turn cases across scope (Bug1 ordinary-day AND emergency-work meal break), out-of-scope (must abstain-or-deflect), meta (capability + conversational recap), overview, coverage (Bug2), tour, and compound (one turn asking two intents). Per-category pass rates; runs as a regression gate. Currently 11/11.
...
- `clause`: `id`, `source`, `clause_number`, `title`, `breadcrumb`, `scope`, `condition`, `page`, `full_text`, `cross_refs`. Rebuilt on every ingest; drives the KB MAP and clause expansion.
...
3. Embedding: build a LlamaIndex `Document` with `id_ = kb_entry_id` and trainer metadata, chunk with `SentenceSplitter(700/150)`, `index.insert_nodes`. Using `id_` as ref_doc_id lets admin removal delete the entry's vectors via `vector_store.delete(ref_doc_id)`.
...
- Admin: `GET|PUT /admin/prompt`; `GET /admin/users`; `POST /admin/users/{id}/role`; `POST /admin/users/{id}/reset-password`; `GET /admin/users/{id}/sessions`; `GET /admin/users/{id}/sessions/{sid}/messages`; `GET /admin/kb`; `DELETE /admin/kb/{id}`.
...
- `app/db.py`: async engine, `async_session_maker`, `Base`, `create_db_and_tables`.
- `app/models.py`: User, ChatSession, ChatMessageRecord, SystemPromptConfig, TrainerKBEntry, Clause; role + kind constants.
...
- `app/rag/chat.py`: default system prompt, condense, `split_into_questions`, `build_search_query`, `build_refined_search_query`, `generate_answer` + `generate_answer_stream`, `verify_answer` (map/source/conversation authority), `build_llm` (answer lane) / `build_fast_llm` (fast lane), citation labels, `summarise_conversation`, `UNSURE_RESPONSE`.
- `app/rag/pipeline.py`: `stream_grounded_answer` (async generator yielding status/delta/reset/final, heartbeat, sub-question retrieval, applicability, agentic re-retrieval) and `produce_grounded_answer` (drives it, returns the final) — the single answer path used by `/chat`, `app.ask`, `app.smoke`, the eval harness.
...
- `app/kb/parse.py`: structure-aware PDF/DOCX parsing (merges split headings; appendix handling; per-page fallback).
...
- `app/kb/ingest_kb.py`: full ingest — parse → situate/chunk → embed to Qdrant + save BM25 + persist clause table. `python -m app.kb.ingest_kb`.
...
- `app/main.py`: app wiring, lifespan (create tables + seed prompt), routers, all endpoints; `/chat` streams `stream_grounded_answer` frames.
...
- `app/admin/page.tsx`: admin panel (users + roles + reset password + view chats; system prompt editor; KB list + delete).
...
## 16. Build / run / deploy
Local (Windows/PowerShell; full detail in `.cursor/manual.md`): `docker compose up -d postgres qdrant`; `pip install -r requirements.txt`; `python -m app.kb.ingest_kb`; `python -m app.seed_admin`; run the backend on the HOST (`uvicorn app.main:app --port 8000 --reload`) because the BM25 index lives on disk in `kb_index/`; `cd frontend && npm install && npm run dev`. Verify: `python -m app.eval_harness` (expect 11/11) and `python -m app.smoke` (no abstentions on Cases 1–12).
...
- Documents: `documents/` (the four files in §3b).
