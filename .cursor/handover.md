...
- `app.eval_harness` = **11/11**. `app.smoke` (Arif's verbatim Cases 1–12 in `.cursor/smokecases.md`) = clean. Re-ingest produces ~446 chunks / ~418 clauses (post-Phase-6 category layout).
...
- **Phase 6 (post-`m1-stable`):** ingestion is now CATEGORY-AWARE + MULTI-FORMAT (`documents/` category folders driven by `objectives.json`; `.pdf`/`.docx`/`.doc`-via-LibreOffice/`.json`/`link_*.txt`/`.txt`; `clause.category`; category-grouped MAP). ALL document-specific lines were stripped from the prompts (generic scope/precedence only) at Arif's direction — the emergency cases now pass on the generic mechanisms alone. Reactive **Gaps** (out-of-scope → logged `KnowledgeGap` + "noted for management" message + admin view). Admin **delete-user** (cascade). Idempotent startup migration + answer-lane transient-overload retry. Proactive Gaps deferred (`.cursor/to_discuss.md`).
- **Known external flake:** Anthropic Opus can return a transient `overloaded_error` (529) mid-stream; the answer lane now retries with backoff (rides it out). Under a sustained overload, just re-run eval/smoke.
...
2b. Reactive gap gate (`classify_gap`, fast): check the standalone question against the KB MAP. Out-of-scope (topic genuinely absent — IT passwords, building codes, personal HR cases) → log a `KnowledgeGap` (when `/chat` passes `user_id`/`session_key`) and return `GAP_RESPONSE`, skipping retrieval. Defaults to in-scope (greetings, capability, overview/tour, recap, any mapped topic) so it never refuses an answerable question. NOT agentic tool-use — the MAP is the deterministic authority (rationale in `.cursor/to_discuss.md`).
...
Ingestion (`app/kb/ingest_kb.py`): `list_categorised_documents()` walks the `objectives.json` category folders (`app/kb/categories.py`) → `parse_document` dispatches by suffix (`.pdf`/`.docx` structure-aware; `.doc`→docx via headless LibreOffice; `.json` flattened; `link_*.txt` single-URL fetch via trafilatura; `.txt`) → thread the folder name as `category` → situate+chunk with generated titles (`app/kb/contextual.py`, fast) → embed to Qdrant + save BM25 corpus (`kb_index/`) + persist clause table (`app/kb/clause_model.py`, incl. `category`). Full rebuild each run; wipes the Qdrant collection (so trainer-added KB vectors are erased). `.doc` files are skipped on a host without LibreOffice (local Windows) and ingested only in the container.
...
13. **Prompts must stay DOCUMENT-AGNOSTIC (Arif's hard rule, Phase 6).** The prompts/instructions must contain NO document names, clause numbers, or topic keywords (the old prompt had an emergency/AIIMS/NES/clause-4.3 paragraph — all removed). A specific scenario must be made to work via GENERIC mechanisms (situating scope at ingest, the applicability filter, the generic scope/precedence rules), never by hardcoding the case into the prompt. If a specific case regresses, fix the generic mechanism and re-measure — do NOT re-couple the prompt to a document. This is the same principle that killed the keyword `detect_scope` hack (§6.1).
...
- `app/rag/pipeline.py` — the one answer path: `stream_grounded_answer` (gap gate + frames + heartbeat + sub-question retrieval + agentic re-retrieval + transient-overload retry; takes `user_id`/`session_key`) and `produce_grounded_answer`.
- `app/gap_store.py` — `KnowledgeGap` helpers (`log_gap`, `list_gaps`, `set_gap_status`, `delete_user_gaps`).
- `app/rag/chat.py` — GENERIC system prompt (no document-specific lines), condense, `split_into_questions`, `build_search_query`, `build_refined_search_query`, `classify_gap`/`GAP_RESPONSE` (reactive Gaps), `generate_answer(_stream)`, `verify_answer` (map/source/conversation authority), `build_llm`/`build_fast_llm`, `passage_label` ("learned from management" for trainer origin), `UNSURE_RESPONSE`.
...
- `app/kb/categories.py` (objectives.json loader), `app/kb/parse.py` (suffix dispatch + all six loaders), `app/kb/contextual.py`, `app/kb/clause_model.py`, `app/kb/bm25_index.py`, `app/kb/ingest_kb.py` (category walk), `app/kb/store_clauses_from_corpus.py` — ingestion.
- `app/main.py` — FastAPI wiring + all endpoints (incl. `DELETE /sessions/{id}`, `DELETE /admin/users/{id}` cascade, `GET /admin/gaps`, `POST /admin/gaps/{id}/status`); `/chat` streams the pipeline (passing `user_id`/`session_id`).
- `app/db_migrate.py` — idempotent startup column migrations (run inside `create_db_and_tables`).
...
- Auth/stores: `app/auth.py`, `app/models.py` (incl. `Clause.category`, `KnowledgeGap`, nullable `trainer_id`), `app/schemas.py`, `app/seed_admin.py`, `app/chat_store.py`, `app/config_store.py`, `app/kb_store.py`, `app/admin_store.py` (`count_admins`, `delete_user_and_data`), `app/gap_store.py`, `app/trainer_kb.py`.
...
- Deploy scripts (run from `app-src/`; they copy `deploy/compose.server.yaml` → `../compose.yaml`):
  ...
  - **Phase 6 deploy specifics:** the image now installs `libreoffice-writer` + `trafilatura`, so the FIRST Phase 6 deploy MUST rebuild (`hard_update.sh`, not a bare restart). The startup column migration (`app/db_migrate.py`) runs automatically inside `create_db_and_tables` — no manual SQL. Because both ingestion and `DEFAULT_SYSTEM_PROMPT` changed, run the re-ingest (`hard_update.sh`) AND `reset_prompt`. `link_*.txt` ingestion makes live outbound HTTP from the container — ensure egress is allowed or those files just skip.
...
0. **Proactive Gaps (DECISION PENDING — `.cursor/to_discuss.md`).** Reactive Gaps shipped (Phase 6). The proactive idea — let the bot/trainer suggest how to restructure the docs and what's missing, fed by the reactive gap log — is parked. Build only after Arif picks an approach (recommended: isolated trainer-only module, zero shared state with the user chat).
...
1. `.doc` (old binary Word) is now ingested by converting to `.docx` via headless LibreOffice (`soffice`) at ingest — but ONLY where LibreOffice exists (the backend Docker image; NOT a bare Windows host, where `.doc` files print `[skip]` and are omitted). So local-Windows ingest silently lacks the `.doc` documents; the container has them. The other new formats (`.json`, `link_*.txt`, `.txt`) work everywhere.
