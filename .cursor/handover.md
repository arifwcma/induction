...
2. Condense history + message → standalone question (`condense_to_standalone_question`).
...
## 6. Key files
- `app/rag/pipeline.py` — the one answer path.
- `app/rag/chat.py` — system prompt, condense, `build_search_query`, generation, verifier, `build_llm` (temp 0 + seed), `UNSURE_RESPONSE`.
- `app/rag/retrieval.py` — dense+BM25 candidate gathering over multiple queries, fuse/dedup, Cohere rerank.
- `app/rag/applicability.py` — conditional-passage filter (primary Bug1 guard).
