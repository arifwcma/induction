...
- nginx (one-time, manual): the `induction.wcma.work` 443 block must route ALL backend paths, not just `/chat`. Matcher: `location ~ ^/(chat|auth|users|sessions|kb|admin|health)`. It also sets `proxy_buffering off` for streaming; consider `proxy_read_timeout 300s; proxy_send_timeout 300s;` there too. Full block + reload in `deploy/README.md`. Without the matcher, login/admin/sessions 404.
